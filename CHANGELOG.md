...
### Fixed
- Legacy projects saved before the enable-style switches became non-responsive deliver those switch props as the strings `"true"`/`"false"` (Elements flattens responsive values into `app.props`), which the strict boolean comparisons introduced alongside `responsive: false` never matched. Symptoms in legacy projects: spacing switched off still emitted margin/padding classes, Min & Max sizing classes were silently dropped, and gradient via stops were dropped. New `shared-hooks/core/switchToBool.js` normalizes a switch prop to a boolean (booleans pass through; `"true"`/`"false"` coerced, base token wins on a breakpoint-prefixed string; anything else returns `undefined` so each call site keeps its own default). All nine switch comparison sites now go through it: `globalSpacing`/`globalSpacingMargin`/`globalSpacingPadding`, `globalSizing`/`globalSizingContainer`, `globalBackground` via stops, and the `globalOverlay` via stops (previously `== 'true'`, now also boolean-ready for when that control goes non-responsive). Rebuild packs with `rw-build hooks`
